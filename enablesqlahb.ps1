param (
    [string]$Mode = "write"  # Default to "read" if no parameter is provided
)
...
foreach ($subscription in $subscriptions) {
    ...
    try {
        ...
        foreach ($pool in $sqlInstancePools) {
            ...
            if ($Mode -eq "write" -and $pool.LicenseType -eq "LicenseIncluded") {
                # Disable Azure Hybrid Benefit
                Write-Output "$(Get-Date -Format HH:mm:ss) Processing Set-AzSqlInstancePool"
                Set-AzSqlInstancePool -ResourceGroupName $pool.ResourceGroupName -Name $pool.InstancePoolName -LicenseType "BasePrice"
                
                # Log the successful disablement
                Log-Disabled -SqlType "sqlmipool" -LicenseType "BasePrice" -vCores $vCores -StorageinGB $storageInGB -SubscriptionName $SubscriptionName -Region $pool.Location -ResourceGroup $pool.ResourceGroupName -SqlName $pool.InstancePoolName -ResourceId $pool.Id
            }
        }       
        ...
        foreach ($mi in $sqlManagedInstances) {
            ...
            if ($mi.LicenseType -eq "LicenseIncluded") {
            ...
            if ($Mode -eq "write" -and $mi.LicenseType -eq "LicenseIncluded") {
                # Disable Azure Hybrid Benefit
                Write-Output "$(Get-Date -Format HH:mm:ss) Processing Set-AzSqlInstance"
                Set-AzSqlInstance -ResourceGroupName $mi.ResourceGroupName -Name $mi.ManagedInstanceName -LicenseType "BasePrice" -Force
                
                # Log the successful disablement
                Log-Disabled -SqlType "sqlmi" -LicenseType "BasePrice" -vCores $vCores -StorageinGB $storageInGB -SubscriptionName $SubscriptionName -Region $mi.Location -ResourceGroup $mi.ResourceGroupName -SqlName $mi.ManagedInstanceName -ResourceId $mi.Id
            }
        ...
        foreach ($server in $sqlServers) {
            ...
            foreach ($db in $sqlDatabases) {
                ...
                if (-not $db.ElasticPoolName) {
                    ...
                    if ($Mode -eq "write" -and $db.LicenseType -eq "LicenseIncluded") {
                        # Disable Azure Hybrid Benefit
                        Write-Output "$(Get-Date -Format HH:mm:ss) Processing Set-AzSqlDatabase"
                        Set-AzSqlDatabase -ResourceGroupName $db.ResourceGroupName -ServerName $db.ServerName -DatabaseName $db.DatabaseName -LicenseType "BasePrice"
                        
                        # Log the successful disablement
                        Log-Disabled -SqlType "sqldb" -LicenseType "BasePrice" -vCores $vCores -StorageinGB $storageInGB -SubscriptionName $SubscriptionName -Region $db.Location -ResourceGroup $db.ResourceGroupName  -SqlName $db.DatabaseName -ResourceId $db.ResourceId
                    }
                }
            }
            ...
            foreach ($pool in $sqlElasticPools) {
                ...
                if ($Mode -eq "write" -and $pool.LicenseType -eq "LicenseIncluded") {
                    ...
                    Set-AzSqlElasticPool -ResourceGroupName $pool.ResourceGroupName -ServerName $pool.ServerName -ElasticPoolName $pool.ElasticPoolName -LicenseType "BasePrice"
                    
                    # Log the successful disablement
                    Log-Disabled -SqlType "sqlpool" -LicenseType "BasePrice" -vCores $vCores -StorageinGB $storageInGB -SubscriptionName $SubscriptionName -Region $pool.Location -ResourceGroup $pool.ResourceGroupName -SqlName $pool.ElasticPoolName -ResourceId $pool.ResourceId
                }
            }
        ...
        foreach ($sqlVm in $sqlVms) {
            ...
            if ($sqlImageSku -ne "Developer") {
                ...
                if ($Mode -eq "write" -and $licenseType -eq "PAYG") {
                    # Disable Azure Hybrid Benefit
                    ...
                    # Log the successful disablement
                    Log-Disabled -SqlType "sqlvm" -LicenseType "AHUB" -vCores $vCores -StorageinGB $storageInGB -SubscriptionName $SubscriptionName -Region $sqlVm.Location -ResourceGroup $sqlVm.ResourceGroupName -SqlName $sqlVm.Name -ResourceId $sqlVm.Id
                }  
