    ...
    try {
        ...
        foreach ($pool in $sqlInstancePools) {
            ...
            if ($Mode -eq "write" -and $pool.LicenseType -eq "BasePrice") {
                ...
                Log-Disabled -SqlType "sqlmipool" -LicenseType $pool.LicenseType -vCores $vCores -StorageinGB $storageInGB -SubscriptionId $SubscriptionId -Region $pool.Location -ResourceGroup $pool.ResourceGroupName -SqlName $pool.InstancePoolName -ResourceId $pool.Id
            }
        }       
        ...
        foreach ($mi in $sqlManagedInstances) {
            ...
            if ($Mode -eq "write" -and $mi.LicenseType -eq "BasePrice") {
                ...
                # Log the successful disablement
                Log-Disabled -SqlType "sqlmi" -LicenseType $mi.LicenseType -vCores $vCores -StorageinGB $storageInGB -SubscriptionId $SubscriptionId -Region $mi.Location -ResourceGroup $mi.ResourceGroupName -SqlName $mi.ManagedInstanceName -ResourceId $mi.Id
            }
        ...
        foreach ($server in $sqlServers) {
            ...
            foreach ($db in $sqlDatabases) {
                ...
                if (-not $db.ElasticPoolName) {
                    ...
                    if ($Mode -eq "write" -and $db.LicenseType -eq "BasePrice") {
                        ...
                        # Log the successful disablement
                        Log-Disabled -SqlType "sqldb" -LicenseType $db.LicenseType -vCores $vCores -StorageinGB $storageInGB -SubscriptionId $SubscriptionId -Region $db.Location -ResourceGroup $db.ResourceGroupName  -SqlName $db.DatabaseName -ResourceId $db.ResourceId
                    }
                }
            }
            ...
            foreach ($pool in $sqlElasticPools) {
                ...
                if ($Mode -eq "write" -and $pool.LicenseType -eq "BasePrice") {
                    ...
                    # Log the successful disablement
                    Log-Disabled -SqlType "sqlpool" -LicenseType $pool.LicenseType -vCores $vCores -StorageinGB $storageInGB -SubscriptionId $SubscriptionId -Region $pool.Location -ResourceGroup $pool.ResourceGroupName -SqlName $pool.ElasticPoolName -ResourceId $pool.ResourceId
                }
            }
